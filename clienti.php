...
while($row = mysqli_fetch_assoc($res_ana)) {
    ...
    // 5) Aggiungo in $provinceSet
    if ($normPr !== '' && !in_array($normPr, $provinceSet)) {
        $provinceSet[] = $normPr;
    }

    // 6) Ora sì, push finale nel tuo array
    $clienti[] = [
        'ints'       => $ints,
        'stato'      => $stato,
        'agente'     => $row['agente']   ?? '',
...
<head>
  ...
  <link
    ...
    integrity="sha512-vmCkfYOXYCgbsPnra366th1n8BlcS+8sGg9E8nTOQBxHySivnHV7SsToMBT2ee6L9GK+desgSWF+5+WK/PQ8QQ=="
    crossorigin=""
  />
  <!-- Leaflet Control Geocoder JS -->
  <script
    src="https://unpkg.com/leaflet-control-geocoder@1.13.0/dist/Control.Geocoder.js"
    integrity="sha512-37lGvzto5tHOJj2E1FevtjVUImOCWXaiyM151KZWQQd9DK7KohNg1DN8SeihSrA3zHRrOH0KlMLV9Fv9vf1wSA=="
    crossorigin=""
  ></script>
...
<div class="d-flex">
  ...
  <div class="main-content">
    ...
    <script>
      var clients = <?= json_encode($mapClients, JSON_UNESCAPED_SLASHES); ?>;
    </script>
      ...
      <div class="row mb-4">
        <div class="col-12">
          <div class="d-flex align-items-center mb-2">
            <label for="mapFilter" class="me-2 fw-semibold">Mostra:</label>
            <select id="mapFilter" class="form-select w-auto">
              <option value="all">Tutti i clienti</option>
              <option value="has1">Acquisto anno <?= $anno_selezionato ?></option>
              <option value="has2">Ultimi 2 anni</option>
